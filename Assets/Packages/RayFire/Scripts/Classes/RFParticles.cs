    ...
    public class RFParticles
    {
        ...
        public static void SetParticleComponents (RayfireRigid scr)
        {
            ...
            if (scr.HasDebris == true)
                for (int i = scr.debrisList.Count - 1; i >= 0; i--)
                {
                    if (scr.debrisList[i] == null)
                        scr.debrisList.RemoveAt (i);
                    // else
                    // {
                    //     if (debrisList[i].HasChildren == true)
                    //         for (int c = debrisList[i].children.Count - 1; c >= 0; c--)
                    //             if (debrisList[i].children[c] == null)
                    //                 debrisList[i].children.RemoveAt (c);
                    // }
                }

            // Get all Debris and initialize
            if (scr.HasDebris == false)
            {
            ...
            // Null check
            if (scr.HasDust == true)
                for (int i = scr.dustList.Count - 1; i >= 0; i--)
                {
                    if (scr.dustList[i] == null)
                        scr.dustList.RemoveAt (i);
                    else
                    {
                        if (scr.dustList[i].HasChildren == true)
        ...
        static ParticleSystem CreateParticleSystem(Transform tm)
        {
            ...
            // Create root
            ps.transform.position   = tm.transform.position;
            ps.transform.rotation   = tm.transform.rotation;
            ps.transform.parent     = tm.transform;
